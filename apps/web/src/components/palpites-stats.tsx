...
export function PalpitesStats({ nomeUsuario }: { nomeUsuario: string | null }) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: reset limit when search changes
  useEffect(() => {
    setVisibleLimit(10);
  }, [search]);

  // Order: Active matches first (chronological ASC), completed matches last (chronological ASC)
